...
if __name__ == '__main__':
    ...
    for n in N:
        ...
        print(f'Uniform: For N = {n}, mean y0 = {sol_mean_y0}, mean y1 = {sol_mean_y1}, var y0 = {sol_var_y0}, var y1 = {sol_var_y1} at t = 10')
        rel_error.append((np.abs(1-sol_mean_y0/ref_mean[0]), np.abs(1-sol_mean_y1/ref_mean[1])))

        # Sample w from a Halton distribution
        w_samples = uniform.sample(size=n, rule='halton')
        odeint_solutions = []
        for w in w_samples:
            params = c, k, f, w
            odeint_solution = discretize_oscillator_odeint(
        ...
        sol_var_y0 = np.var([sol[0] for sol in odeint_solutions])
        sol_var_y1 = np.var([sol[1] for sol in odeint_solutions])
        print(f'Halton: For N = {n}, mean y0 = {sol_mean_y0}, mean y1 = {sol_mean_y1}, var y0 = {sol_var_y0}, var y1 = {sol_var_y1} at t = 10')
        rel_error_halton.append((np.abs(1-sol_mean_y0/ref_mean[0]), np.abs(1-sol_mean_y1/ref_mean[1])))

    fig = plt.figure(figsize=(10,6), dpi=300)
    plt.loglog(N,[err[0] for err in rel_error], label='Standard')
    plt.loglog(N,[err[0] for err in rel_error_halton], label='Halton')
    plt.title('Relative error of the integral approximations vs N')
    plt.xlabel('N')
    plt.ylabel('Relative error')
    plt.grid()
    plt.legend()
    fig.savefig('assignment_4.2.png')
